...
//			if( !eregi("^.. [GZ] ", $buffer) )
		...
		function burst_channels()
		{
			foreach( $this->channels as $key => $c )
			{
				...
				if( !empty($banlist) )
				{
					$this->sendf( FMT_BURST_BANLIST, SERVER_NUM, 
						$c->get_name(),
						$c->get_ts(),
		...
		{
			$gline_key = strtolower( $gline_key );
		...
		function main()
		{
			...
			while( is_resource($this->sock) && in_array($err_no, $noncritical_socket_errors) )
			{
				...
				debug( "*** Socket timeout: $timeout secs");
				...
				$rcv_to = socket_get_option($this->sock, SOL_SOCKET, SO_RCVTIMEO);
				$snd_to = socket_get_option($this->sock, SOL_SOCKET, SO_SNDTIMEO);
/*
				debug("Receive timeout: $rcv_to[sec].$rcv_to[usec]s" );
				debug("   Send timeout: $snd_to[sec].$snd_to[usec]s" );
*/			
				$this->service_preread();
				$num_changed_socks = socket_select( $sock_array, $sock_array, $except = NULL, $timeout, 0 );
				debug( "*** Woke up ($num_changed_socks)" );
				
				if($num_changed_socks !== false)
				{
					$tmp_buffer = '';
					// $buffer .= @socket_read( $this->sock, 1024 );
					@socket_recv( $this->sock, $tmp_buffer, 1024, 0);
					$buffer .= $tmp_buffer;
					$this->bytes_received += strlen( $tmp_buffer );
				}
				
				foreach( $this->timers as $n => $timer )
				{
					if( $timer->get_next_run() <= time() )
					{
						$this->run_timer( $n );
					}
				}
		...
		function perform_chan_user_mode( $chan_name, $mode_pol, $mode_char, $arg_list )
		{
			...
			for( $i = 0; $i < count($arg_list); ++$i )
			{
				...
				{
					$this->sendf( FMT_MODE_HACK, SERVER_NUM, 
						$chan->get_name(), 
		...
		{
			if( TOPIC_BURSTING )
				$this->sendf( FMT_TOPIC, $this->get_numeric(), $chan_name, time(), $topic );
			else
				$this->sendf( FMT_TOPIC, $this->get_numeric(), $chan_name, $topic );
		}
